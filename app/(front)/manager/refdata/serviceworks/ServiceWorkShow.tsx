...
const arrToShow = (enteredArr: any) => {
  const localArr = structuredClone(enteredArr);
  ...
  const transformedArr = localArr.map((currentItem: any) => {
    ...
    for (const element of currentItem.serviceWorkGroup) {
      serviceWork__ToString += `${element.serviceWorkGroupName}, `;
    }

    for (const element of currentItem.products) {
      products__ToString += `${element.productName}, `;
    }

    for (const element of currentItem.inventars) {
      inventars__ToString += `${element.productName}, `;
    }

    for (const element of currentItem.tools) {
      tools__ToString += `${element.productName}, `;
    }

    for (const element of currentItem.equipment) {
      equipment__ToString += `${element.productName}, `;
    }

    for (const element of currentItem.workerProtection) {
